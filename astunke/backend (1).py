...
NODE_DIR = SCRIPT_DIR / 'node_exec'

app = FastAPI(title='Patas Localhost Stack', version='0.2.0')
app.add_middleware(
    CORSMiddleware,
    allow_origins=['*'],
...
requests_session.headers.update({'Content-Type': 'application/json', 'User-Agent': 'PatasLocalhost/0.2'})
...
def fetch_curve_snapshot(mint: str) -> Dict[str, Any]:
    ...
    return {'mint': mint, 'curve': curve, 'name': meta.get('name'), 'symbol': meta.get('symbol'), 'decoded': decoded}
...
def apply_snapshot(snapshot: Dict[str, Any]):
    ...
    with state_lock:
        ...
        if vs != previous_vs:
            state['trade_history'].append({'time': time.time(), 'type': 'buy' if vs > previous_vs else 'sell', 'delta_sol': round(abs(vs - previous_vs), 6)})
            state['last_activity'] = time.time()
        state['trade_history'] = trim_history(state['trade_history'])
        state.update({
            'mint': snapshot['mint'], 'curve': snapshot['curve'], 'name': snapshot.get('name'), 'symbol': snapshot.get('symbol'),
            'v_sol': vs, 'v_tok': vt, 'r_sol': rs, 'graduated': bool(decoded.get('complete')),
            'note': 'GRADUATED - liquidity migrated to PumpSwap' if decoded.get('complete') else None,
            'last_poll': time.time(), 'failure_count': 0, 'mode': 'live'
        })
...
def start_demo_monitor(mint: str):
    stop_current_monitor()
    with state_lock:
        state.update({'mint': mint, 'curve': 'DEMO_CURVE', 'name': 'Demo Token', 'symbol': 'DEMO', 'v_sol': 32.0, 'v_tok': 998_000_000.0,
                      'r_sol': 1.85, 'sol_price': fetch_sol_usd(), 'failure_count': 0, 'last_activity': time.time(), 'last_poll': time.time(),
                      'graduated': False, 'note': 'Demo mode active', 'trade_history': [], 'mode': 'demo'})
        ...
        while not stop_event.is_set():
            with state_lock:
                ...
                if is_trade:
                    ...
                    state['trade_history'].append({'time': time.time(), 'type': 'buy' if buy else 'sell', 'delta_sol': round(delta, 6)})
                    state['last_activity'] = time.time()
                state['trade_history'] = trim_history(state['trade_history'])
...
def get_active_metrics() -> Dict[str, Any]:
    ...
    current['metrics'] = {
        ...
        'buys_60s': buys, 'sells_60s': sells, 'tension': tension, 'tiers': tiers,
    }
    return current
...
def ensure_node_runtime() -> str:
    ...
    if not node or not npm:
        raise RuntimeError('Node.js and npm are required for Jupiter execution')
    pkg = NODE_DIR / 'package.json'
    if not pkg.exists():
        raise RuntimeError('node_exec/package.json missing')
    marker = NODE_DIR / 'node_modules'
    if not marker.exists():
        subprocess.run([npm, 'install'], cwd=str(NODE_DIR), check=True)
    return node
...
def run_jupiter_execution(mint: str, sol_in: float, slippage: float, priority_fee: float) -> Dict[str, Any]:
    node = ensure_node_runtime()
    script = NODE_DIR / 'jupiter_exec.mjs'
    env = provider_env()
    cmd = [node, str(script), mint, str(sol_in), str(slippage), str(priority_fee)]
    proc = subprocess.run(cmd, cwd=str(NODE_DIR), capture_output=True, text=True, env=env)
    if proc.returncode != 0:
        raise RuntimeError(proc.stderr.strip() or proc.stdout.strip() or 'Jupiter execution failed')
    return json.loads(proc.stdout)


def run_pump_fallback(mint: str, sol_in: float, slippage: float, priority_fee: float) -> Dict[str, Any]:
    env = load_env()
    private_key = env.get('PRIVATE_KEY') or env.get('TRADING_PRIVATE_KEY')
    endpoint = env.get('PUMPFUN_BUY_API') or 'https://pumpfunapis.com/api/buy'
    if not private_key:
        raise RuntimeError('PRIVATE_KEY missing for Pump.fun fallback')
    payload = {
    ...
    if env.get('RPC_URL'):
        payload['rpc'] = env['RPC_URL']
    resp = requests.post(endpoint, json=payload, timeout=20)
    if resp.status_code >= 400:
        raise RuntimeError(resp.text[:500])
    data = resp.json()
    return {'provider': 'pumpfun', 'raw': data, 'signature': data.get('signature') or data.get('txid') or data.get('txHash')}
...
def health():
    ...
    return {
        'ok': True,
        'rpcConfigured': bool(helius_rpc_url()),
        'privateKeyConfigured': bool(env.get('PRIVATE_KEY') or env.get('TRADING_PRIVATE_KEY')),
        'nodeConfigured': bool(shutil.which('node')),
    }
...
def execute_buy(req: ExecuteRequest):
    current = get_active_metrics()
    mint = current.get('mint')
    if not mint:
        raise HTTPException(400, 'No active mint monitored yet')
    if current.get('graduated'):
        raise HTTPException(400, 'Curve graduated, localhost curve execution disabled')
    if current.get('mode') in ('idle', 'stale'):
        raise HTTPException(400, 'Monitor is idle or stale')

    tiers = current['metrics']['tiers']
    tier_key = req.tier.lower()
    if tier_key not in ('t1', 't2'):
        raise HTTPException(400, 'tier must be t1 or t2')
    if tier_key == 't2' and not req.confirmT2:
        raise HTTPException(400, 'confirmT2 must be true for T2 execution')

    sol_in = round(float(tiers[tier_key]), 4)
    max_cap = 0.25
    if sol_in > max_cap:
        raise HTTPException(400, f'Tier size {sol_in} exceeds local cap {max_cap} SOL')
    ...
    try:
        if req.dryRun:
            execution.update({'status': 'simulated', 'message': 'Dry run only'})
        else:
            try:
                out = run_jupiter_execution(mint, sol_in, req.slippage, req.priorityFee)
                execution.update({'status': 'submitted', 'provider': 'jupiter', 'signature': out.get('signature'), 'message': out.get('message', 'Jupiter execution ok')})
            except Exception as first_exc:
                fallback = run_pump_fallback(mint, sol_in, req.slippage, req.priorityFee)
                execution.update({'status': 'submitted', 'provider': 'pumpfun', 'signature': fallback.get('signature'), 'message': f'Jupiter failed, fallback used: {first_exc}'})
    except Exception as exc:
        execution.update({'status': 'failed', 'message': str(exc)})
        with state_lock:
            state['execution_log'].append(execution)
            state['execution_log'] = state['execution_log'][-20:]
        raise HTTPException(500, execution['message'])
